...
class GradientWeightEstimator(WeightEstimator):
    ...
    @abstractmethod
    def train(self, data_tr: Sequence[Trajectory], data_cal: Sequence[Trajectory], lr: float, epochs: int) -> Tuple[NDArray[np.float64], NDArray[np.float64]]:
        scores = []
        weights = []
...
class _WeightsEstimator(object):
    def __init__(self, behaviour_policy: Policy, pi_star: Policy, lower_quantile_network: MLP, upper_quantile_network: MLP):
        self.behaviour_policy = behaviour_policy
        self.pi_star = pi_star
        self.lower_quantile_network = lower_quantile_network
    ...
    def empirical_avg_method(self, data_tr: Sequence[Trajectory], data_cal: Sequence[Trajectory],
                             num_states: int, num_rewards: int) -> Tuple[NDArray[np.float64], NDArray[np.float64], None]:
        weights_estimator = np.zeros((num_states, num_rewards))
        num_visits = np.ones_like(weights_estimator)
        for traj in data_tr:
            x,y = traj.initial_state, int(traj.cumulative_reward)
            weight = compute_weights_gradient(traj, self.behaviour_policy, self.pi_star) 
            num_visits[x,y] += 1
            weights_estimator[x,y] += weight
            
        mask = np.argwhere(num_visits > 0)
        weights_estimator[mask] = weights_estimator[mask] / num_visits[mask]
        weights_estimator[~mask] = 1 / len(data_tr)

        calibration_weights = []
        for traj in data_cal:
            x,y = traj.initial_state, int(traj.cumulative_reward)
            w = weights_estimator[x,y]
            calibration_weights.append(w)
